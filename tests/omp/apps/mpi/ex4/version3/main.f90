PROGRAM SOR3
  ...
  ! In this example, the four-color SOR method is used.
...
!      x(r,c) = r + c*(rows+2) + myrank*(rows+2)*localCols
  ...
  DO step = 1, 300 
    ...
    ! Update first half 
    CALL synchronize(0)
    DO c = 1+even, localCols, 2
      DO r = 1, rows, 2
        temp = 0.125 * (x(r,c-1) + x(r-1,c) + x(r+1,c) + x(r,c+1) + &
                        x(r-1,c-1) + x(r+1,c-1) + x(r-1,c+1) + x(r+1,c+1)) &
               - x(r,c)
    ...
    DO c = 1+odd, localCols, 2
      ...
      ENDDO
    ENDDO
    CALL printGrid("After third update")
    DO c = 1+odd, localCols, 2
      DO r = 2, rows, 2
        temp = 0.125 * (x(r,c-1) + x(r-1,c) + x(r+1,c) + x(r,c+1) + &
                        x(r-1,c-1) + x(r+1,c-1) + x(r-1,c+1) + x(r+1,c+1)) &
      ...
      ENDDO
    ENDDO
    CALL printGrid("After fourth update")
