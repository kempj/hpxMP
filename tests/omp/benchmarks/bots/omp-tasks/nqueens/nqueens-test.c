...
//It seems entirely possible to convert the allocas and arrays to a single int, 
// and some atomic increments for the solution.
// It will also require tracking of depth, so that calculation of all states with
// a state that has been determined invalid will not continue.
...
#pragma omp single
        {
            char *a = alloca(size * sizeof(char));
